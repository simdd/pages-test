...
// Note: crypto is imported at the top level using ESM import
function md5(text) {
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.headers.host === 'localhost:9000') {
      ...
      // 如果 referer 存在，则使用 referer
      // 否则使用 x-forwarded-proto 和 host
      if (referer) {
        if (referer !== '/' && referer.endsWith('/')) {
          referer = referer.slice(0, -1);
        }
        fullPath = referer + req.url;
      } else {
        fullPath = xForwardedProto + '://' + host + req.url;
      }
